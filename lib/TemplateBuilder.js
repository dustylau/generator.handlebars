...
class TemplateBuilder {
  ...
  constructor(files, replacements, ignoreList, includeList) {
    this._files = files && Array.isArray(files) ? files : files ? [files] : [];
  ...
  addFile(file) {
    if (!this._files.find((f) => f === file)) this._files.push(file);
  }

  addFiles(directoryPath, recurse, ignoreList, includeList) {
    var files = FileHelper.getFilesSync(directoryPath, recurse, {
  ...
  buildSingleFile(
    ...
    addTemplateScript =
      addTemplateScript || TemplateBuilder.defaultAddTemplateScript;
  ...
  buildSingleFileSettings(fileName, settings) {
    ...
    settings.ExportPath =
      settings.ExportPath || TemplateBuilder.defaultExportPath;
    settings.FileNamePattern =
      settings.FileNamePattern || TemplateBuilder.defaultFileNamePattern;
    settings.RemoveFileName =
      settings.RemoveFileName || TemplateBuilder.defaultRemoveFileName;
    settings.SplitOn = settings.SplitOn || TemplateBuilder.defaultSplitOn;

    this.writeSettingFile(fileName, settings);
  }

  buildIndividualFiles(directoryPath, _addTemplateScript) {
    _addTemplateScript =
      _addTemplateScript || TemplateBuilder.defaultAddTemplateScript;
    const fileInformations = FileHelper.loadFilesSync(this._files);
    for (const file of fileInformations) {
  ...
  buildIndividualFile(directoryPath, file, addTemplateScript) {
    ...
    this.buildIndividualFileSettings(settingsFileName, {
