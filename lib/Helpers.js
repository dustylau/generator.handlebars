const camelCase = function(value) {
    ...
    var lastCharacterIsUpper = true;
    var camelCaseComplete = false;
    ...
    for (let index = 0; index < value.length; index++) {
        ...
        if (isLowerCase(character)){
            result += character;
            lastCharacterIsUpper = false;
            camelCaseComplete = true;
            continue;
        }

        const nextCharacter = value[index + 1];
        const nextCharacterIsUpper = !isLowerCase(nextCharacter);

        if (lastCharacterIsUpper && !nextCharacterIsUpper) {
            result += character;
